...
- **Starta:** Kör `run.bat` (eller `mvn javafx:run`). Kräver Java 27 och Maven — JavaFX hanteras automatiskt av Maven.
- Inloggning: **admin** / **admin123**
...
>
> **Körsteget bygger sitt eget CDS-arkiv.** Körsteget är `liberica-openjre-alpine:27`, och till skillnad från Temurin levereras den **utan** JDK:ns förbyggda klassarkiv (`lib/server/classes.jsa`, Class Data Sharing), så varje JDK-klass laddas kallt vid start. På Renders gratis-CPU tog Spring-kontexten då 28 s att initiera (1,1 s lokalt), och den första Java 27-deployen föll på `Port scan timeout reached, no open ports detected`: Render slutade leta efter porten innan Tomcat hann lyssna. Bygget och databasen var hela hela tiden. Lösningen är två rader i Dockerfilen: `RUN java -Xshare:dump` bygger arkivet en gång i avbildningen, och `-XX:TieredStopAtLevel=1` (bara C1-kompilatorn) kortar starten ytterligare på en CPU-snål instans. Sedan dess går deployen igenom. Byts körsteget tillbaka till Temurin kan båda raderna tas bort. Obs: en fallen deploy syns inte utifrån, eftersom Render fortsätter servera den gamla avbildningen — kontrollera deployloggen.
---
